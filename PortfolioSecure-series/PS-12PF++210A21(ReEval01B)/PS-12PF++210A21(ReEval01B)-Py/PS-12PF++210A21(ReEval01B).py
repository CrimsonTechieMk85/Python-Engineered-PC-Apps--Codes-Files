...
app_name = "PS 12 PF++ (210A2.1-type)"
...
''' Main App detials '''
# App details
app_title = "Portfolio Secure 12 Perfected.Further + + (210 frame A2.1-type)"

# database folder
main_app_folder_name = 'PS-12PF++210A2.1'
    ...
    def encDir(self):
        ...
        try:
            ...
            if dir_name_in != "" or dir_root_in != "":
                ...
                if os.path.exists(full_dir_path) == False:
                    ...
                    try:
                        if os.path.exists(compressed_dir_path_enc) == False:
                            ...
                            ''' Asks user whether to close for assurance as message box. if yes, clears the values for the user '''
                            yn_clr_msgbox = wx.MessageDialog(self.wpanel, "Do you wish to encrypt another folder?",
                                                             "Clear - Yes/No", wx.YES_NO | wx.ICON_NONE)
                ...
                invalid_err_ver = wx.MessageDialog(self.wpanel, "Input invalid.", "Input verifier error:",
                                                   wx.ICON_ERROR | wx.STAY_ON_TOP)

                invalid_err_ver.ShowModal()
...
class FileGuardian(wx.Frame):
    ...
    def encryption(self):
        ...
        try:
            ...
            complete_file_rt = os.path.join(self.src_in_rt, self.src_file_full_name)

            src_fl_exists = os.path.exists(complete_file_rt)

            if src_fl_exists is False:

                # print("\n- Requested file '{}' is not found at the current location.".format(file_full_name))

                msg_file_missing = wx.MessageDialog(self.wpanel,
                                                    "OOps! couldn't find the file '{0}' at the requested location ({1}).".format(
                                                        self.src_file_full_name, self.src_in_rt),
                                                    "{} file detector error - File not found:".format(app_name),
                                                    wx.ICON_ERROR | wx.STAY_ON_TOP)

                msg_file_missing.ShowModal()
                ...
                try:
                    ...
                    if self.src_file_name == '' or self.src_file_format == self.format_lists[0] or self.src_in_rt == '':
                        ...
                        yn_msgbox = wx.MessageDialog(self.wpanel, "Do you wish to encrypt another file ?", "Yes/No",
