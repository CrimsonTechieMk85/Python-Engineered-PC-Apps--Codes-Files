...
app_name = "PS 12 PEF++ (210F3.6.3-2C)"
...
''' Main App detials '''
# App details
app_title = "Portfolio Secure 12 Perfected.Even.Further ++ (210-F3.6.3-2C)"

# GUI messages heading app title
# sub_app_title = 'Portfolio Secure 12 Perfected.Even.Further ++ (210-F3.6)'

# database folder
app_db_folder_name = 'PS-12PEF++210F363-2C'
...
main_app_folder_name = 'Portfolio Secure 12 P.E.F ++ (210F3.6.3-2C)'
...
''' Json data write a file - function'''
def json_infos_dump(json_flpath, info):
    json_w = open(json_flpath, "w")

    json.dump(info, json_w)
...
def enc_dec_folder_shortcut_maker_dt(app_main_folder_root_var, years_sub_folder_root_var_enc,
                                     years_sub_folder_root_var_dec, date_sub_folder_root_var_enc,
                                     date_sub_folder_root_var_dec, subfolder2_root_var_encfiles,
                                     subfolder2_root_var_decfiles, subfolder2_root_var_encdirs,
...
class appUI(wx.Frame):
    ...
    def __init__(self, parent, id):
        ...
        try:
            ...
            password_json_overwrite(json_path_val=passwords_json_file_path, enc_pswrd_var=encrypt_passcode_no_encrypted,
                                    dec_pswrd_var=decrypt_passcode_no_encrypted)
        except FileNotFoundError:
            pass
    ...
    def dir_opts_dlg(self):
        ...
        if settings_json_paths_exists == False:
            ...
            if dir_dlgbox.ShowModal() == wx.ID_OK:
                ...
                json_infos_dump(json_flpath=paths_datas_json_file_path, info=usr_path)
                    ...
                    if self.cipher_pass_input.GetValue() == self.encrypt_pass:
                        ...
                        if enc_filedir_dlgbox.ShowModal() == wx.ID_OK:
                            ...
                            try:
                                ...
                                json_infos_dump(json_flpath=json_file_backups_file_path,info=json_src_data)
                                ...
                                # Tranfers data And then stores it on the json file
                                json_infos_dump(json_flpath=json_file_backups_file_path,info=backup_fl_infos)
                    ...
                    if self.cipher_pass_input.GetValue() == self.encrypt_pass:
                        ...
                        if enc_filedir_dlgbox.ShowModal() == wx.ID_OK:
                            ...
                            try:
                                ...
                                # Tranfers data And then stores it on the json file
                                json_infos_dump(json_flpath=json_folder_backups_file_path,info=backup_dirs_infos)
    ...
    def pswrds_changer(self):
        ...
        fg_modify_pswrds_lists = ['Choose here to change password for encryption.           ',
                                  'Choose here to change password for decryption.       ']
                            ...
                            if self.cipher_pass_input.GetValue() == self.encrypt_pass:
                                ...
                                if self.modify_cipher_pass_input.ShowModal() == wx.ID_OK:
                                    ...
                                    if self.modify_cipher_pass_input.GetValue() == "":
                                        ...
                                        json_src_data["encrypt_passcode"] = cipher.encrypt(
                                            bytes(str(self.modify_cipher_pass_input.GetValue()), 'utf-8')).decode()

                                        # json_src_data["encrypt_passcode"] = self.modify_cipher_pass_input.GetValue()

                                        json_infos_dump(json_flpath=passwords_json_file_path,info=json_src_data)
                            ...
                            if self.decipher_pass_input.GetValue() == self.decrypt_pass:
                                ...
                                if self.modify_decipher_pass_input.ShowModal() == wx.ID_OK:
                                    ...
                                    if self.modify_decipher_pass_input.GetValue() == "":
                                        ...
                                        json_src_data["decrypt_passcode"] = cipher.encrypt(
                                            bytes(str(self.modify_decipher_pass_input.GetValue()), 'utf-8')).decode()
            ...
            if modify_dir_dlgbox.ShowModal() == wx.ID_OK:
                ...
                if usr_rt_v == "":
                    ...
                    try:
                        ...
                        json_infos_dump(json_flpath=paths_datas_json_file_path,info=json_src_data)
...
class FolderGuardian(wx.Dialog):
    ...
    def __init__(self, parent, id):
        ...
        self.name1_lbl = wx.StaticText(self.wpanel, -1, "Portfolio Secure 12 P.E.F ++ (210-F3.6.3-2C) \nFolder Guardian",
                                       (67, 85), (24, 24),
                            ...
                            if self.cipher_pass_input.GetValue() == self.encrypt_pass:
                                ...
                                if self.modify_cipher_pass_input.ShowModal() == wx.ID_OK:
                                    ...
                                    if self.modify_cipher_pass_input.GetValue() == "":
                                        ...
                                        json_infos_dump(json_flpath=passwords_json_file_path,
                                                        info=json_src_data)
            ...
            if modify_dir_dlgbox.ShowModal() == wx.ID_OK:
                ...
                if usr_rt_v == "":
                    ...
                    try:
                        ...
                        json_infos_dump(json_flpath=paths_datas_json_file_path,
                                        info=json_src_data)
                        ...
                        # File Guardafolder-shorcut maker
                        enc_dec_folder_shortcut_maker_dt(app_main_folder_root_var=self.collections_folder_path,
                                                         years_sub_folder_root_var_enc=self.crnt_yrs_folder_path_enc,
                                                         years_sub_folder_root_var_dec=self.crnt_yrs_folder_path_dec,
                                                         date_sub_folder_root_var_enc=self.now_date_folder_enc_path,
                ...
                try:
                    ...
                    try:
                        if os.path.exists(compressed_dir_path_enc) == False:
                            ...
                            json_infos_dump(json_flpath=json_folder_backups_file_path, info=backup_fl_infos)
            ...
            if modify_dir_dlgbox.ShowModal() == wx.ID_OK:
                ...
                if usr_rt_v == "":
                    ...
                    try:
                        ...
                        json_infos_dump(json_flpath=paths_datas_json_file_path, info=usr_rt_v)
