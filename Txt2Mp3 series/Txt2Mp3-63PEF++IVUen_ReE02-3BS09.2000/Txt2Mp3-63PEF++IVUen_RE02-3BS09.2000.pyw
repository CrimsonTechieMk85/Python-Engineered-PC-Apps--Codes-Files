...
def appMp3s_foldershortcut_maker_dt(root,rootdt):
...
class app_ui(wx.Frame):
    ...
    def change_dir(self, event):
        ...
        if dir_dlgbox.ShowModal()==wx.ID_OK:
            ...
            now_date_sub_folder_v = os.path.join(app_audio_folder_v,dt_date)  

            appMp3s_foldershortcut_maker_dt(root=app_audio_folder_v,rootdt=now_date_sub_folder_v)
    ...
    def open_mp3_folder_dt(self,event):
        ...
        if settings_json_fl_exists == False:
            ...
            if dir_dlgbox.ShowModal()==wx.ID_OK:
                ...
                settings_path_saved_msg =wx.MessageDialog(self.wpanel,"Path updated.","Path update - Complete",wx.OK | wx.ICON_INFORMATION )
    ...
    def gtts_V(self):
        ...
        ''' Variable AppName audio folder path '''
        app_audio_folder_v = os.path.join(src_path_data,et_audio_app_folder_name)
