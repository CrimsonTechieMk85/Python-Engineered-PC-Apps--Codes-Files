...
# -----------------------------------------------------------------------------------------
''' App Details '''
# Dynamic [Early-Tests type] app title PF+
et_app_title = "Txt2Mp3-6.3 PEF++ I.V.U-en [early-tests.Id: {}]".format(file_name)

# App title
app_title = "Txt-2-Mp3 6.3 (Perfected.Even.Further++ Revised-2BB) [Individual.Variable.Utility-enhanced]"
...
main_folder_name = 'Txt-2-Mp3 6.3 (P.E.F++ Rev-2BB) [I.V.U-en]'
audio_app_folder_name = '{} audios'.format(main_folder_name)
...
# time delay seconnds for each sequence or activities
...
class app_ui(wx.Frame):
    ...
    # Access folder button
        ...
        self.open_folder_btn.SetToolTip("Click here to access audios collections.")# sets features pop-up details
    ...
    # Play Mp3 button
        ...
        self.play_btn = wx.Button(self.wpanel,label='Play Mp3-3',pos=(334,404),size=(156,45),style=wx.BORDER_RAISED)
        ...
        self.Bind(wx.EVT_BUTTON,self.playMP33,self.play_btn)# combines button with its functions
    ...
    # Make Main folder & shortcut
    def appMp3s_foldershortcut_maker_dtrev2BB(self,path1, path2, path3, path4, app_folder_name4):
        ...
        fllnk_path = os.path.join(desktop, '{} - Shortcut.lnk'.format(app_folder_name4))

        if os.path.exists(fllnk_path) == False:
            target_rt = path1
            wDir_rt = path1
            shell = Dispatch('WScript.Shell')
            shortcut = shell.CreateShortCut(fllnk_path)
    ...
    #  Read json file data
    def read_json(self,jsonfl_path2,keys2):

        with open(jsonfl_path2,'r') as jsonfl2_r:
            data = jsonfl2_r.read()
            json_infos_v = json.loads(data)[keys2]

        return json_infos_v

    # Update the existing value in a json file
    def update_json(self, jsonfl_path3,keys3, infos3):

        with open(jsonfl_path3, 'r') as jsonfl3_r:
            json_src_data = json.load(jsonfl3_r)

            json_src_data[keys3] = infos3

            with open(jsonfl_path3, 'w') as  json_fl3_w:

                json.dump(json_src_data, json_fl3_w)

    # Save data in a json file
    def save_json(self, jsonfl_path1, keys1, infos1):

        data = {
            keys1: infos1
        }

        with open(jsonfl_path1, 'w') as json_fl1_w:
            json.dump(data, json_fl1_w)

    # gTTS api audios generator
    def gtts_audios_generate(self,text_val, lang_val, tdl_val, audio_root_val):

        gtts_audios = gTTS(text=text_val, lang=lang_val, slow=False, lang_check=True, tld=tdl_val)

        gtts_audios.save(audio_root_val)

    def dirs_dlgbox_savepath(self):

        if os.path.exists(settings_json_fl_rt) == False:
            ...
            if dir_dlgbox.ShowModal() == wx.ID_OK:
                ...
                # Saving path into a save json function
                self.save_json(jsonfl_path1=settings_json_fl_rt, keys1="path", infos1=usr_rt_v)
    ...
    def change_dir(self, event):
        ...
        if dir_dlgbox.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                # Upating\Changing path using upadte json function
                self.update_json(jsonfl_path3=settings_json_fl_rt,keys3="path",infos3=usr_rt_v)

                # Getting the read json function
                returned_path = self.read_json(jsonfl_path2=settings_json_fl_rt,keys2="path")

                # App main folder path
                app_main_folder_path = os.path.join(returned_path, audio_app_folder_name)

                # Current date folder path
                now_yrs_sub_folder_path = os.path.join(app_main_folder_path, dt_yyyy)

                # Current month number-month name folder path
                ...
                self.appMp3s_foldershortcut_maker_dtrev2BB(path1=app_main_folder_path, path2=now_yrs_sub_folder_path,
                                                     path3=now_mnthsno_months, path4=now_date_sub_folder_path,
                                                     app_folder_name4=audio_app_folder_name)
    ...
    def playMP33(self,event):

        self.dirs_dlgbox_savepath()

        if os.path.exists(settings_json_fl_rt) == True:
            ...
            while True:
                ...
                files_dlgbox = wx.FileDialog(self.wpanel, "Please choose audio to be played from '{}' folder:".format(audio_app_folder_name), now_date_sub_folder_path,"",
                                             wildcard="MP3 (*.mp4) |*.mp3;",
                                             style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)

                if files_dlgbox.ShowModal() == wx.ID_OK:
                    # Choosen Video file parametres
                    self.file_choosen = files_dlgbox.GetPath()

                    video_name, video_format = os.path.splitext(os.path.basename(self.file_choosen))

                    playsound(self.file_choosen)
    ...
    def open_mp3_folder_dt(self,event):
        ...
        if os.path.exists(settings_json_fl_rt) == True:
            ...
            self.appMp3s_foldershortcut_maker_dtrev2BB(path1=app_main_folder_path,path2=now_yrs_sub_folder_path,path3=now_mnthsno_months,path4=now_date_sub_folder_path,app_folder_name4=audio_app_folder_name)
    ...
    def txt2Mp3(self,event):
        ...
        if os.path.exists(settings_json_fl_rt) == True:
            ...
            # Getting the read json function
            self.returned_path = self.read_json(jsonfl_path2=settings_json_fl_rt, keys2="path")
    ...
    def gtts_V(self):
        ...
        # App main folder path
        app_main_folder_path = os.path.join(self.returned_path, audio_app_folder_name)
        ...
        if settings_json_fl_exists == True:
            ...
            try:
                ...
                audio_full_path = os.path.join(now_date_sub_folder_path,audio_name_mp3)
                
                if os.path.exists(audio_full_path)==False:
                   
                    self.gtts_audios_generate(text_val=str_txtin,lang_val=str_langin,tdl_val=str_tdlin,audio_root_val=audio_full_path)
            ...
            # Active when there is no text to convert
                ...
                saltr_msg = wx.MessageDialog(self.wpanel,"Seeking alternative.... Please press 'Ok' to continue.","Notify",wx.OK)
                ...
                try:
                    ...
                    with m as source:
                        sr_get_notify_msg = wx.MessageDialog(self.wpanel,"You can speak now after clicking 'Ok'.","Text-2-Mp3 Voice capture mode: Google-Speech Recognition",wx.OK)
                    ...
                    try:
                        ...
                        audio_name_mp3_sr = "{0}-{1},{2}-{3}_sr.mp3".format(str(str_sr_var),str_langin,str_tdlin,"audio")

                        audio_full_path_sr = os.path.join(now_date_sub_folder_path,audio_name_mp3_sr)

                        sr_rec_msg = wx.MessageDialog(self.wpanel,"Speech recognized..","Text-2-Mp3 Voice capture mode: Google-Speech Recognition",wx.OK)

                        sr_rec_msg.ShowModal()

                        if os.path.exists(audio_full_path_sr)==False:

                            self.gtts_audios_generate(text_val=str(str_sr_var),lang_val=str_langin,tdl_val=str_tdlin,audio_root_val=audio_full_path_sr)
