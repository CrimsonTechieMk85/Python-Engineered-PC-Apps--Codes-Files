...
label_name.grid(row=10,column=2,sticky=N)

fl_name = Entry(Midframe, relief=SUNKEN,bg="white",fg="red")
fl_name.grid(row=10,column=3,sticky=N,padx=0, pady=0,ipadx=200,ipady=0)

lbl = Label(Midframe, text = " ")
lbl.grid(row=13,column=1,sticky=N)

label_path = Label(bottomFrame,text ="Path of the file: ",fg="blue")
label_path.grid(row=13,column=2,sticky=N)

fl_path = Entry(bottomFrame, relief=SUNKEN,bg="white",fg="blue")
fl_path.grid(row=13,column=3,sticky=N,padx=0, pady=0,ipadx=200,ipady=0)

lbl = Label(bottomFrame, text = " ")
lbl.grid(row=20,column=2,sticky=E)


button_fl = Button(bottomFrame,text="Input",fg="white",bg="green",command=V_2_A)
button_fl.grid(row=23,column=3,padx=0, pady=0,ipadx=68,ipady=0) 


lbl_btn_fl = Label(bottomFrame, text = " ")
lbl_btn_fl.grid(row=26,column=2)


button_clr = Button(bottomFrame, text="clear",fg="white",bg="blue", command=clr_btn)
button_clr.grid(row=29,column=3,padx=0, pady=0,ipadx=68,ipady=0) 

lbl_btn_clr = Label(bottomFrame, text = " ")
lbl_btn_clr.grid(row=32,column=3,sticky=E)
...
image_fl = os.getcwd()+"\\MUsic_apps\\Crimson Beauty.jpg"
fp = open(image_fl,"rb")
img = PIL.Image.open(fp)
photo=ImageTk.PhotoImage(img)
lab=Label(topFrame,image=photo)
lab.grid(row=12,column=4,padx=0, pady=0,ipadx=68,ipady=0)

lbl_btn_clr = Label(bottomFrame, text = " ")
lbl_btn_clr.grid(row=14,column=4,sticky=E)
